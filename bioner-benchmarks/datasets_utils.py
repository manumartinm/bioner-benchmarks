...
def from_hf_tokens_to_valid(
    df: pd.DataFrame, entity_map_args: Optional[Dict[int, str]]
) -> pd.DataFrame:
    if entity_map_args is None:
    ...
    for index, row in df.iterrows():
        ...
        for i, token in enumerate(tokens):
            new_row = {
                "words": token,
                "labels": entity_map_args[ner_tags[i]],
...
def parse_bcdr_to_format(
    df: pd.DataFrame, entity_map_args: Optional[Dict[int, str]], model: Optional[str]
) -> pd.DataFrame:
    new_data = []
    checkpoint_path = model if model else "NeuML/pubmedbert-base-embeddings"

    tokenizer = AutoTokenizer.from_pretrained(
        checkpoint_path, use_fast=True, do_lower_case=True
    )
    ...
    for index, row in df.iterrows():
        for passage in row["passages"]:
            ...
            token_offsets = tokenizer(text, return_offsets_mapping=True)[
                "offset_mapping"
            ]
...
def from_hf_to_pubtator(
    df: pd.DataFrame,
    entity_map_args: Optional[Dict[int, str]],
    ...
    for index, row in df.iterrows():
        ...
        for i, (ner_tag, token) in enumerate(zip(ner_tags, tokens)):
            entity_label = entity_map_args[ner_tag]
            if entity_label.startswith("B-"):
                if entity:
                    line += (
                        f"{sentence_id}\t{start}\t{i}\t{entity}\t{entity_label[2:]}\n"
                    )
                entity = token
                start = i
...
def parse_bcdr_to_pubtator(
    df: pd.DataFrame,
    entity_map_args: Optional[Dict[int, str]] = None,
    add_entities: Optional[bool] = True,
) -> List[str]:
    new_data = []
        ...
        for passage in row["passages"]:
            ...
            for entity in passage.get("entities", []):
                ...
                document_entities.append(
                    f"{document_id}\t{start_offset}\t{end_offset}\t{entity_text}\t{entity_type}\n"
                )
...
entity_map = {0: "O", 1: "I-Entity", 2: "B-Entity"}
